 ...
 * begin commit abort create drop insert select delete update where show ....
 */

public class Parser {

    /**
     * 将statement包装成具体的statement类并返回
     * 核心就是一个调用 Tokenizer 类分割 Token，并根据词法规则包装成具体的 Statement 类并返回。解析过程很简单，仅仅是根据第一个 Token 来区分语句类型，并分别处理
     */
    public static Object Parse(byte[] statement) throws Exception {
        Tokenizer tokenizer = new Tokenizer(statement);
        String token = tokenizer.peek();    //返回最前面的一个token
        tokenizer.pop();

        //解析最前面的token，根据第一个token来区分语句类型
        Object stat = null;
